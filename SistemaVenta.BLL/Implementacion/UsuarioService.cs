...
namespace SistemaVenta.BLL.Implementacion
{
    public class UsuarioService : IUsuarioService
    {
        ...
        public async Task<bool> CambiarClave(int IdUsusario, string ClaveActual, string ClaveNueva)
        {
            try
            {
                Usuario usuario_encontrado = await _repository.Obtener(u=>u.IdUsuario== IdUsusario);

                if (usuario_encontrado == null)
                    throw new TaskCanceledException("El usuario no existe");

                if (usuario_encontrado.Clave != _utilidadesService.ConvertirSha256(ClaveActual))
                    throw new TaskCanceledException("Contraseña ingresada como actual incorrecta");
        ...
        public async Task<bool> RestablecerClave(string Correo, string UrlPlantillaCorreo)
        {
            try
            {
                ...
                if (!correo_enviado)
                    throw new TaskCanceledException("Tenemos problemas.\nPor favor intentalo luego.");
